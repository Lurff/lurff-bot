...
const data = {
    ...
    execute(interaction) {
        ...
        user.timeout(0,reason).then(() => {
            ...
            interaction.client.channels.cache.get(mod).send({embeds:[Log]})

            return interaction.reply({embeds:[embed("",`${user} Adlı Kullanıcı **"${reason}"** Sebebiyle Susturması Kaldırılmıştır`,"#5865F2")]})
